...
import 'package:vendue_vendor/src/models/tokendata_model.dart';
...
class _ChatState extends State<FireChat> {
  ...
  _getUSer() async {
    ...
    if (tokenModel.responseCode == "1") {
      userToken = tokenModel.user.deviceToken;
      print('?>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>?');
      print(userToken);
    }
    print(responseData);
              ...
              builder: (context, snapshot) {
                if (!snapshot.hasData) {
                  return Center(
                      child: CircularProgressIndicator(
                          valueColor:
                              AlwaysStoppedAnimation<Color>(appColorYellow)));
                } else {
  ...
  Future<void> onSendMessage(String content, int type) async {
    ...
    if (content.trim() != '') {
      ...
      sendNotification(userToken, content);
